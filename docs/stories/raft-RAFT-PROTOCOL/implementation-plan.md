...
#### Implementation Steps
- [ ] Create `xraft-core/src/traits.rs` defining `#[async_trait] trait LogStore { async fn append(&self, entries: &[LogEntry]) -> Result<()>, async fn read(&self, start_offset: u64, end_offset: u64) -> Result<Vec<LogEntry>>, async fn truncate_suffix(&self, from_offset: u64) -> Result<()>, async fn truncate_prefix(&self, up_to_offset: u64) -> Result<()>, fn log_start_offset(&self) -> u64, fn log_end_offset(&self) -> u64, async fn entry_at(&self, offset: u64) -> Result<Option<LogEntry>> }` — all methods take `&self` with interior mutability (e.g., `tokio::sync::Mutex<File>`) and `Sync` bound; matches architecture §4.1 trait definition and IoStage concurrent dispatch model
...
- [ ] Define `RaftNode::read() -> Result<ConsensusState>` method returning the current committed protocol state (current term, leader, high_watermark, role, voter set, log_end_offset, node_id) — `read()` returns the current `ConsensusState` by cloning the latest value from `self.state_rx` (`tokio::sync::watch::Receiver<ConsensusState>`, architecture §5.11); the event loop is the sole writer — it calls `watch_tx.send(node_state.project())` after each state mutation (step 4 in per-message processing order); `read()` never reads the `LogStore` directly, never enters the event loop's message queue, and never contacts other nodes; the returned state reflects the latest HW-committed protocol position; the high_watermark in the returned state is an exclusive upper bound (entry at offset O is committed when O < HW, per architecture §3.1); callable on any node (leader, follower, candidate, unattached); linearisable reads (read-index, lease-based) are out of scope per tech-spec §2.2
...
- [ ] Define `RaftNode::shutdown() -> Result<()>` lifecycle method that invokes `Listener::begin_shutdown()` synchronously within the event loop, drops the ReceiverTask's mpsc sender (causing the ReceiverTask to exit per architecture §4.4), drains pending I/O, and stops the event loop; awaits both `receiver_task_handle` and `event_loop_handle` to ensure clean shutdown
...
#### Implementation Steps
- [ ] Create `xraft-core/src/node_state.rs` defining the internal `NodeState` struct (`pub(crate)` visibility) with all fields from architecture §3.2: `node_id`, `cluster_id`, `current_term`, `voted_for`, `role` (Unattached | Follower | Candidate | Leader), `leader_id`, `log_start_offset`, `log_end_offset`, `high_watermark`, `voter_set: Vec<VoterInfo>`, `observers: HashSet<NodeId>`, `pending_membership_change: Option<PendingMembershipChange>`, `follower_state: HashMap<NodeId, FollowerProgress>`, `election_deadline`, `votes_received`, `pre_votes_received`, `check_quorum_deadline` — also define `PendingMembershipChange { offset: u64, voters: Vec<VoterInfo> }` for tracking uncommitted `VotersRecord` entries; add a method `NodeState::project(&self) -> ConsensusState` that produces the public projection (the 7 fields: `node_id`, `current_term`, `role`, `leader_id`, `log_end_offset`, `high_watermark`, `voter_set`)
- [ ] Create `xraft-core/src/event_loop.rs` with async event loop skeleton: inbound message channel (`tokio::sync::mpsc`), tick-based timer, shutdown signal, and a `tokio::sync::watch::Sender<ConsensusState>` for publishing state updates to `RaftNode::read()` callers (architecture §5.11) — the event loop calls `watch_tx.send(node_state.project())` after each state mutation (step 4 in the per-message processing order from architecture §2.1); wire the `watch::Receiver` end to `RaftNode` for `read()` (Stage 5.3)
- [ ] Create `xraft-core/src/receiver_task.rs` implementing the `ReceiverTask` per architecture §4.4 — a dedicated async task that owns `Box<dyn TransportReceiver>` and a `mpsc::Sender<RpcEnvelope>`; in a loop, calls `TransportReceiver::recv(&mut self)` and forwards the received `RpcEnvelope` into the event loop's mpsc channel; the ReceiverTask performs no protocol logic — only deserialise and forward; it does NOT call `Clock`, `LogStore`, `QuorumStateStore`, `SnapshotIO`, `TransportSender`, or application callbacks; spawned by `RaftNode` after recovery/bootstrap completes (§4.4 startup sequencing), stopped by dropping the mpsc sender during `RaftNode::shutdown()`
