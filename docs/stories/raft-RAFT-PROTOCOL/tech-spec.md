...
| **High watermark (HW)** | The leader tracks each voter's replication progress via `fetch_offset` (the next offset the follower wants — equivalently, its `log_end_offset`). A follower with `fetch_offset = N` has replicated entries `[0, N)`. The leader collects one value per voter (`fetch_offset` for followers, `log_end_offset` for itself), sorts them in **descending** order, and picks the value at index `⌊V/2⌋` (0-indexed, where V = total voters). This is the highest offset at which at least a majority have replicated. HW advances to `max(current_HW, candidate)` — it never decreases. HW is an **exclusive upper bound**: entry at offset O is committed ⟺ `O < HW`. Equivalently, `HW − 1` is the last committed offset. HW = 0 means no entries are committed. HW is never persisted to disk — it is recovered from the leader's state after restart (see §2.1.7 Crash recovery). Only voters count toward HW; observers do not contribute to quorum. | Red Hat article "Raft replication" step 3; architecture §3.1 |
...
| **Voter records** | Membership changes are committed via a `VotersRecord` control record in the log (analogous to KRaft's `VotersRecord`). The record encodes the **complete** new voter set (not a delta). **Dual quorum semantics** apply once a `VotersRecord` is appended: (1) **HW advancement** for entries at or after the `VotersRecord`'s offset immediately uses the **new** (pending) voter set — the new voter's `fetch_offset` counts toward commit of the `VotersRecord` itself; (2) **Elections**, **Check Quorum**, and `read().voter_set` continue to use the **committed** voter set until the `VotersRecord` is committed; (3) On commit, the committed `voter_set` is atomically replaced by the new set and the `pending_membership_change` is cleared; (4) If log truncation discards an uncommitted `VotersRecord` (e.g., after leader failover), the pending state is cleared and the node reverts to the committed voter set. The committed voter set is also persisted in snapshot metadata for recovery. | Red Hat article "Dynamic quorum" — VotersRecord; architecture §5.5 |
...
| **Leader step-down** | If the leader is removed from the new configuration, it continues serving as leader (processing Fetch requests, advancing HW using the **new** voter set for entries at and after the `VotersRecord` offset) until the `VotersRecord` commits. Commitment requires a majority of the **new** voter set (which excludes the departing leader). Once committed, the leader transitions to `Unattached` (not `Follower`, since it is no longer a member of the voter set) and stops accepting proposals. The remaining voters' election timers fire and a new leader is elected from the new configuration. | Red Hat article "Dynamic quorum"; architecture §5.6 |
...
| **`read() → Result<ConsensusState>`** | Returns a local, non-linearizable snapshot of the node's consensus protocol metadata. `ConsensusState` is a **separate, smaller public struct** — a projected subset of the internal `NodeState` — containing: `node_id`, `current_term`, `role`, `leader_id`, `log_end_offset`, `high_watermark`, and `voter_set` (the **committed** voter set only — pending membership changes are internal and not exposed). Internal-only fields (`cluster_id`, `voted_for`, `log_start_offset`, `observers`, `pending_membership_change`, `follower_state`, election/quorum deadlines, vote counters) are excluded. The event loop updates a `tokio::sync::watch` channel with the latest `ConsensusState` after each state mutation; `read()` clones the current value from this channel, making it lock-free and callable from any thread. Operationally infallible (the `Result` wrapper exists for API consistency; it always succeeds). Callable on any node — leader, follower, candidate, or unattached. Does **not** read application state; applications build their own read-side state from committed records delivered via `Listener::handle_commit` (see architecture §5.11). |
...
| **Persistent quorum state** | Each node persists a `QuorumState` record in a `quorum-state` file separate from the log. The file contains four fields: `current_term: Term`, `voted_for: Option<NodeId>`, `leader_id: Option<NodeId>`, and `leader_epoch: Term`. This file is read at startup before any RPCs are processed and before log replay. The `leader_id` and `leader_epoch` fields allow a restarted node to know the last known leader without waiting for a Fetch response. Note: KRaft's `votedDirectoryId` mechanism is intentionally not adopted — xraft uses the unique-NodeId constraint (§4.2) instead. | Red Hat article "Metadata management" — `quorum-state` file; architecture §3.2 `QuorumState` struct |
| **Crash recovery** | On restart, a node recovers by: (1) reading the `quorum-state` file for `current_term`, `voted_for`, `leader_id`, and `leader_epoch`; (2) loading the most recent snapshot (if any) — restoring the committed voter set from snapshot metadata and the application state via `StateMachine::restore`; (3) scanning log entries after the snapshot offset to rebuild internal metadata only: updating `log_end_offset`, rebuilding the leader-epoch checkpoint from `LeaderChangeMessage` entries, and detecting any uncommitted `VotersRecord` (stored as `pending_membership_change`, **not** applied to the committed `voter_set`); (4) **not** re-applying post-snapshot `Command` entries to the state machine — committed entries are already reflected in the snapshot, and entries beyond the last-known HW may be uncommitted and subject to truncation; (5) setting HW to the snapshot's `last_included_offset + 1` as a conservative floor — the authoritative HW is learned from the leader on the first successful `Fetch` response; (6) transitioning to `Follower` role and starting the election timer. The node does **not** accept proposals or participate in elections until recovery completes and the `ReceiverTask` is spawned. | Raft paper §5.2; KRaft recovery model; architecture §5.10 |
