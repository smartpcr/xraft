...
│  │  Accumulator │──│  (inline     │  │  Queue (park/complete     │  │
│  │  (stage      │  │   batch I/O) │  │   client futures)         │  │
│  │   proposals) │  │              │  │                           │  │
│  └──────────────┘  └──────┬───────┘  └───────────────────────────┘  │
└──────────┬──────────────────┬────────────────────┬───────────────────┘
           │                  │                    │
...
`io_stage.execute()` — slow I/O delays processing of the next queued
message — but the `ReceiverTask` (§4.4) continues to receive and queue
...
pause well within typical election timeouts (150–300 ms).
...
6. **Log integrity via CRC-32C.** Each batch in the log is checksummed.
   ...
   acknowledged to any peer — in the pull-based (Fetch) model, the leader
   ...
   retained with **unknown** committed status — they are NOT truncated by
...
> (see §7.3 R4). **Active divergence:** the impl plan's "routes reads through
> the log" wording implies `read()` calls `LogStore::read()`, which is
> incorrect — `read()` clones from a `tokio::sync::watch` channel and never
> touches the `LogStore`. The impl plan must be corrected to describe the
> watch-channel mechanism. See §7.4 D2 for the required fix.
...
| **I/O trait objects** | Storage / Network-Send I/O traits injected as `Box<dyn ...>` — no `Arc`. Trait objects are moved into the `IoStage` at construction; the `IoStage` is moved into the event loop task. `IoStage::execute(&self)` is called inline by the event loop (not a separate task) — the event loop pauses during I/O. Within a batch, actions run concurrently via `tokio::join!` (total pause = max latency, not sum). Trait objects are borrowed via `&self` with `Sync` bound for concurrent dispatch. | impl plan Stage 1.7. Tech spec §4.4.1's "prevents slow I/O from delaying Fetch handling" is overstated — see §7.3 R10. |
...
| **Node roles** | Four roles: `Unattached`, `Follower`, `Candidate`, `Leader`. `Unattached` is the initial state before bootstrap/recovery and the terminal state for removed voters. | architecture §2.1/§3.2, impl plan Stage 1.2 (`Role { Unattached, Follower, Candidate, Leader }`), e2e preamble (line 17). Tech spec §2.1.1 summarises the Raft paper's three steady-state roles; xraft extends the set with `Unattached` — a KRaft-specific state (see §7.2 resolution). |
| **Callback execution model** | Application callbacks (`StateMachine::apply`, `Listener::handle_commit`) are synchronous, in-process calls invoked by the `EventLoop` during message processing, after state mutation but before `IoAction` dispatch. | tech spec §4.4.1, architecture §4.1, impl plan Stages 4.1/5.1, e2e Client Interaction. **Active divergence:** tech spec §6 decision table (Event-loop pattern row) still contains stale wording stating "application callbacks are offloaded to async tasks" — this directly contradicts the tech spec's own §4.4.1 body, this architecture, the impl plan, and the e2e scenarios, all of which specify synchronous in-process dispatch. See §7.4 D1 for the required corrective edit. |
...
| **Node role count** | Four roles: `Unattached`, `Follower`, `Candidate`, `Leader` (§2.1, §3.2). | Tech spec §2.1.1 lists "Three states: Follower, Candidate, Leader" (Raft paper reference). | Resolved: the tech spec §2.1.1 row summarises the Raft paper's three steady-state roles. xraft adds `Unattached` as a KRaft-specific state for pre-bootstrap and post-removal. The impl plan (Stage 1.2), e2e scenarios (preamble line 17), and this architecture all define four roles. The tech spec's "three states" is a source-material description, not a constraint on xraft's design. Implementation uses four roles. |

> **Note:** The callback dispatch model divergence in tech spec §6 (previously listed here) is reclassified as an **active divergence** in §7.4 D1 because the stale wording remains on disk in the tech spec and cannot be treated as resolved.
...
accounted for. Items R5 and R12 are **active divergences** requiring
sibling-doc updates (see §7.4). Item R11 has been resolved (see §7.2).
Item R13 identifies a **gap** (unspecified callback error semantics) filled
by this architecture. Where a divergence remains, this architecture document
is canonical.
...
| **R11** | **Node role count** | This architecture defines **four** roles: `Unattached`, `Follower`, `Candidate`, `Leader` (§2.1, §3.2). `Unattached` is the initial state before bootstrap or recovery completes, and the terminal state for a node removed from the voter set (§5.6). | Tech spec §2.1.1 says "Three states: Follower, Candidate, Leader." E2e scenarios (line 17) and impl plan (Stage 1.2) both list all four roles. | **Resolved (see §7.2).** The tech spec §2.1.1 row summarises the Raft paper's three steady-state roles — it is a source-material reference, not a constraint on xraft's design. xraft extends the set with `Unattached`, a KRaft-specific state for pre-bootstrap and post-removal. Three of four documents explicitly define four roles; the tech spec's three-role mention is an abbreviation of the source material. `Unattached` is a distinct state: the node does not participate in elections, does not accept proposals, and does not send Fetch requests. Implementation uses four roles per this architecture. |
| **R12** | **Callback dispatch model in tech-spec decision table** | Application callbacks (`StateMachine::apply`, `Listener::handle_commit`) are **synchronous, in-process calls** within the event loop — they are NOT offloaded to async tasks (§4.1). The event loop invokes them after mutating `NodeState` (step 2) but before producing the `IoAction` batch (step 3). | Tech spec §6 decision table (Event-loop pattern row) says "application callbacks are offloaded to async tasks." Tech spec §4.4.1 body (line 292) correctly says callbacks are invoked "synchronously within the loop (in-process calls, not external I/O)." | **Active divergence (see §7.4 D1).** The tech spec's §6 decision table retains stale wording ("offloaded to async tasks") that contradicts its own §4.4.1 body, this architecture, the impl plan, and the e2e scenarios. The tech spec §6 row must be corrected to say "invoked synchronously within the loop." See §7.4 D1 for the specific wording fix. |
...
### 7.4 Active Divergences Requiring Sibling-Doc Updates

The following contradictions exist on disk in the sibling documents and have
**not** been resolved by prior iterations. Each entry specifies the exact
text that must be changed. This architecture document defines the canonical
design; implementors should follow this document where the sibling docs
contradict it.

| ID | Sibling doc | Location | Current (incorrect) wording | Required correction | Canonical reference |
|----|-------------|----------|-----------------------------|---------------------|---------------------|
| **D1** | **tech-spec.md** | §6 decision table, "Event-loop pattern" row, Recommendation column | "consensus state mutations are synchronous within the loop; disk I/O, snapshot streaming, and **application callbacks are offloaded to async tasks**." | Replace with: "consensus state mutations are synchronous within the loop; **application callbacks (`StateMachine::apply`, `Listener::handle_commit`) are invoked synchronously within the loop as in-process calls, not offloaded to async tasks**; disk I/O, snapshot streaming, and network sends are offloaded to the `IoStage` which executes them concurrently." | This architecture §4.1 (three-phase commit notification), §2.1 (EventLoop processing order steps 1–3). Tech spec's own §4.4.1 body (line 292) already says "synchronously within the loop (in-process calls, not external I/O)" — the §6 table contradicts its own detailed section. The impl plan (Stages 4.1/5.1) and e2e scenarios (Client Interaction) both confirm synchronous dispatch. |
| **D2** | **implementation-plan.md** | Stage 1.7 (`RaftNode::read()` definition) and Stage 5.3 (`read()` public API implementation) | "per tech-spec §2.1.5 the initial implementation **routes reads through the log** for safety, meaning the returned state reflects the latest HW-committed position in the log" | Replace with: "`read()` returns the current `ConsensusState` by **cloning the latest value from a `tokio::sync::watch` channel** (architecture §5.11). The event loop is the sole writer — it updates the watch channel after each state mutation (step 4 in the per-message processing order). `read()` never reads the `LogStore` directly, never enters the event loop's message queue, and never contacts other nodes. The returned state reflects the latest HW-committed protocol position because the event loop updates the watch channel after processing log commits." | This architecture §5.11 (Client Read), §2.1 (EventLoop step 4). The tech spec §2.1.5 correctly says `read()` returns a "local, non-linearizable snapshot of the node's consensus state" — this is compatible with the watch-channel mechanism. The impl plan's "routes reads through the log" implies `read()` calls `LogStore::read()`, which is incorrect — `read()` clones from the watch channel and performs no I/O. |
