...
> (see §7.3 R4). **Resolved (iteration 35):** the impl plan's "routes reads
> through the log" wording has been corrected to describe the watch-channel
> mechanism — `read()` clones from a `tokio::sync::watch` channel and never
> touches the `LogStore`. See §7.2.
...
| **Callback execution model** | Application callbacks (`StateMachine::apply`, `Listener::handle_commit`) are synchronous, in-process calls invoked by the `EventLoop` during message processing, after state mutation but before `IoAction` dispatch. | tech spec §4.4.1, tech spec §6 decision table (updated iteration 35), architecture §4.1, impl plan Stages 4.1/5.1, e2e Client Interaction. All four documents now agree on synchronous in-process dispatch. |
...
| **`read()` semantics** | `read() → Result<ConsensusState>` — clones the latest value from a `tokio::sync::watch` channel updated by the event loop after each state mutation. Local, non-linearizable projected subset of protocol metadata (term, role, leader_id, HW, log_end_offset, voter set, node_id). Callable on any node. Does NOT read from `LogStore`, does NOT enter the event loop message queue. Does not read application state. No `StateMachine::query()` method. | tech spec §2.1.5, architecture §5.11, impl plan Stages 1.7/5.3 (updated iteration 35), e2e Client Interaction. All four documents now agree: `read()` clones from the watch channel and never touches the `LogStore`. Sibling docs list 5 core fields (term, role, leader_id, HW, voter set); this architecture adds `log_end_offset` and `node_id` as supplementary fields — abbreviated, not contradictory (§7.3 R4). |
...
| **Callback dispatch in tech-spec §6 decision table** | Application callbacks are synchronous, in-process calls within the event loop (§4.1). | Tech spec §6 decision table previously said "application callbacks are offloaded to async tasks." | Tech spec §6 updated (iteration 35): now reads "application callbacks (`StateMachine::apply`, `Listener::handle_commit`) are invoked synchronously within the loop as in-process calls, not offloaded to async tasks; disk I/O, snapshot streaming, and network sends are offloaded to the `IoStage`." |
| **`read()` implementation mechanism** | `read()` clones the latest `ConsensusState` from a `tokio::sync::watch` channel (§5.11). Never reads from `LogStore`. | Impl plan Stages 1.7 and 5.3 previously said "routes reads through the log for safety." | Impl plan updated (iteration 35): now reads "`read()` clones the latest value from a `tokio::sync::watch` channel that the event loop updates after each state mutation (architecture §5.11); `read()` never reads the `LogStore` directly." |

### 7.3 Cross-Document Reconciliation Notes

The following items were identified as potential divergences between the four
planning documents. All have been analysed and reconciled — the documents
are **compatible** once the differing levels of abstraction and notation are
accounted for. Items R5 and R12 were previously active divergences; they have
been resolved on disk in iteration 35 (see §7.2 and §7.4). Item R11 was
resolved in an earlier iteration (see §7.2). Item R13 identifies a **gap**
(unspecified callback error semantics) filled by this architecture. Where a
divergence remains, this architecture document is canonical.
...
| **R5** | **`read()` implementation path** | `read()` clones the latest `ConsensusState` from a `tokio::sync::watch` channel. The event loop is the sole writer (updates the channel after each state mutation, step 4 in §2.1). `read()` never reads the `LogStore` directly, never enters the event loop's message queue, and never contacts other nodes. The returned state reflects the latest HW-committed protocol position because the event loop updates the watch channel after processing log commits (§5.11). | Impl plan Stages 1.7 and 5.3 previously said "routes reads through the log for safety." Updated in iteration 35. | **Resolved (see §7.2).** The impl plan now correctly describes the watch-channel mechanism: "`read()` clones the latest value from a `tokio::sync::watch` channel that the event loop updates after each state mutation (architecture §5.11); `read()` never reads the `LogStore` directly." |
...
| **R12** | **Callback dispatch model in tech-spec decision table** | Application callbacks (`StateMachine::apply`, `Listener::handle_commit`) are **synchronous, in-process calls** within the event loop — they are NOT offloaded to async tasks (§4.1). The event loop invokes them after mutating `NodeState` (step 2) but before producing the `IoAction` batch (step 3). | Tech spec §6 decision table previously said "application callbacks are offloaded to async tasks." Updated in iteration 35. Tech spec §4.4.1 body (line 292) already said "synchronously within the loop (in-process calls, not external I/O)." | **Resolved (see §7.2).** The tech spec §6 decision table now correctly reads: "application callbacks (`StateMachine::apply`, `Listener::handle_commit`) are invoked synchronously within the loop as in-process calls, not offloaded to async tasks; disk I/O, snapshot streaming, and network sends are offloaded to the `IoStage`." The tech spec §6 table now matches §4.4.1 body, this architecture, the impl plan, and the e2e scenarios. |
...
### 7.4 Resolved Divergences (Previously Active)

All previously active divergences have been resolved on disk in the sibling
documents as of iteration 35. The corrections are recorded in §7.2 and the
corresponding reconciliation notes (R5, R12) in §7.3.

| ID | Sibling doc | Location | Correction applied (iteration 35) | Canonical reference |
|----|-------------|----------|------------------------------------|---------------------|
| **D1** | **tech-spec.md** | §6 decision table, "Event-loop pattern" row | Replaced "application callbacks are offloaded to async tasks" with "application callbacks (`StateMachine::apply`, `Listener::handle_commit`) are invoked synchronously within the loop as in-process calls, not offloaded to async tasks; disk I/O, snapshot streaming, and network sends are offloaded to the `IoStage` which executes them concurrently." | This architecture §4.1 (three-phase commit notification), §2.1 (EventLoop processing order steps 1–3). Tech spec §4.4.1 body already said "synchronously within the loop" — the §6 table now matches. |
| **D2** | **implementation-plan.md** | Stage 1.7 (`RaftNode::read()` definition) and Stage 5.3 (`read()` public API implementation) | Replaced "routes reads through the log for safety" with "`read()` clones the latest value from a `tokio::sync::watch` channel that the event loop updates after each state mutation (architecture §5.11); `read()` never reads the `LogStore` directly, never enters the event loop's message queue, and never contacts other nodes." | This architecture §5.11 (Client Read), §2.1 (EventLoop step 4). |
