...
| **Callback execution model** | Application callbacks (`StateMachine::apply`, `Listener::handle_commit`) are synchronous, in-process calls invoked by the `EventLoop` during message processing, after state mutation but before `IoAction` dispatch. | tech spec §4.4.1, architecture §4.1, impl plan Stages 4.1/5.1, e2e Client Interaction. Tech spec §6 decision table contains stale wording ("offloaded to async tasks") — the tech spec's own §4.4.1 body is the authoritative description and agrees with synchronous dispatch (see §7.2 resolution). |
...
| **Callback dispatch model** | Synchronous, in-process calls within the event loop (§4.1). | Tech spec §6 decision table says "application callbacks are offloaded to async tasks." | Resolved: the tech spec contains an internal inconsistency — its §4.4.1 body (the detailed design section) correctly says callbacks are "synchronously within the loop (in-process calls, not external I/O)," but the §6 decision table row retains stale wording from an earlier design iteration. The tech spec's §4.4.1 body is the authoritative description within the tech spec itself. All four documents' detailed design sections agree on synchronous dispatch. Implementation follows the synchronous model defined here. |
...
accounted for. Items R11 and R12 were previously identified as divergences
and have been resolved (see §7.2 Verified Resolutions). Item R13 identifies
a **gap** (unspecified callback error semantics) filled by this architecture.
...
| **R12** | **Callback dispatch model in tech-spec decision table** | Application callbacks (`StateMachine::apply`, `Listener::handle_commit`) are **synchronous, in-process calls** within the event loop — they are NOT offloaded to async tasks (§4.1). The event loop invokes them after mutating `NodeState` (step 2) but before producing the `IoAction` batch (step 3). | Tech spec §6 decision table (Event-loop pattern row) says "application callbacks are offloaded to async tasks." Tech spec §4.4.1 body (line 292) correctly says callbacks are invoked "synchronously within the loop (in-process calls, not external I/O)." | **Resolved (see §7.2).** The tech spec contains an internal inconsistency: its §4.4.1 body (the authoritative detailed design section) correctly describes synchronous callbacks, while the §6 decision table row retains stale wording from an earlier design iteration. The tech spec's own detailed section (§4.4.1) agrees with this architecture's §4.1, the impl plan's Stages 4.1/5.1, and the e2e scenarios. All four documents' detailed design sections converge on synchronous dispatch. The §6 table row is an editorial artefact, not a design position. Callbacks MUST be synchronous: they observe fully-updated protocol state before any `IoAction` is dispatched, ensuring the three-phase commit notification ordering (§4.1). Implementation follows the synchronous model. |
